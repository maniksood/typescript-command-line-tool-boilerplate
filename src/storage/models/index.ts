...
function constructAllModels() {
  ...
  //
  const db_claimsapp_conf = config.get('db_claimsapp');
  Models['Jarvis1'] = readFilesAndInitialize(db_claimsapp_conf, 'claimsapp');

  const jarvis2 = config.get('jarvis2');
  Models['Jarvis2'] = readFilesAndInitialize(jarvis2, 'jarvis2');

  const info = config.get('info');
  Models['Info'] = readFilesAndInitialize(info, 'info');
  // console.log(Models);
  return Models;
}
